...
#[tauri::command]
pub async fn start_bitcoind(reindex:bool, networkactive:bool) -> Result<String, String> {
	...
	if !output.status.success() {
		return Err(format!("ERROR opening .bitcoin permissions = {}", std::str::from_utf8(&output.stderr).unwrap()));
	}
	...
	if networkactive == true{
		...
		//failure condition, internal drive not properly mounted
			...
			if a == false {
				...
				if !output.status.success() {
					return Err(format!("ERROR in starting bitcoin daemon with creating ../sensitive/wallets dir = {}", std::str::from_utf8(&output.stderr).unwrap()));
				}
			}
		}
	...
	if networkactive == false{
		...
		return Ok(format!("SUCCESS in starting bitcoin daemon with networking disabled"));
		...
		Ok(format!("SUCCESS in starting bitcoin daemon"))
	}
...
pub async fn export_psbt(progress: String) -> Result<String, String>{
	...
	Ok(format!("SUCCESS in Creating transferCD"))
}
